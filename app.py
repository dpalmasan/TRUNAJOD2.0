...
from TRUNAJOD.IndicesVeliz import IndicesVeliz
...
TRUNAJOD_ORDERED_INDICES = [
    ...
    "silabas_por_palabra",
    "letras_por_palabra",
    "densidad_clausulas_finita",
    "densidad_clausulas",
    "promedio_longitud_oracion",
    "promedio_longitud_clausula",
    "palabras_antes_raiz",
    "densidad_FN",
    "diversidad_lexica_MTLD",
    "similaridad_sintactica",
    "densidad_lexica",
    "indice_concrecion",
    "indice_frecuencia",
    "incidencia_NOUN|PROPN",
    "incidencia_NOUN",
    "incidencia_ADJ",
    "incidencia_ADV",
    "incidencia_VERB|AUX",
...
def process_text(text):
    ...
    (
        ...
        adv_lemmas,
        prp_lemmas
    ) = utils.getSentencesLemmas(docs, lemmaDict, stopwords)

    N = len(list(processed_text.sents))
    docs = processed_text.sents

    # This dict will contain TRUNAJOD computed indices
    trunajod_indices = IndicesVeliz(processed_text)
    ...
    trunajod_indices["lexical_overlap"] = semantic_measures.overlap(
        function_lemmas
        + content_lemmas,
        {}
    )
    ...
    trunajod_indices["pronoun_density"] = giveness.pronounDensity(
        processed_text
    )

    trunajod_indices["pronoun_noun_ratio"] = giveness.pronounNounRatio(
        processed_text
    )
    ...
    trunajod_indices["content_ttr"] = ttr.simple_ttr(
        utils.flatten(
            content_lemmas
        )
    )

    trunajod_indices["function_ttr"] = ttr.simple_ttr(
        utils.flatten(
            function_lemmas
        )
    )
    ...
    (
        ...
        lc_pw_dist,
        lc_pacc_dist
    ) = get_local_coherence(entity_grid)
    ...
    trunajod_indices["revision_dm"] = discourse_markers.get_revision_dm_count(
        text
    )
    
    trunajod_indices["cause_dm"] = discourse_markers.get_cause_dm_count(text)
    trunajod_indices["equality_dm"] = discourse_markers.get_equality_dm_count(
        text
    )

    trunajod_indices["context_dm"] = discourse_markers.get_context_dm_count(
        text
    )

    (
        trunajod_indices["polysemic_dm"]
    ) = discourse_markers.get_polysemic_dm_count(text)

    (
        trunajod_indices["closed_class_words"]
    ) = discourse_markers.get_closed_class_vague_meaning_count(text)
...
def main(argv):
    ...
    dataset = pd.read_excel("/mnt/c/Users/dpalma/Desktop/trunajod_experimentos/dataset_trunajod.xlsx")
    df = pd.DataFrame()
    TRUNAJOD_ORDERED_INDICES.insert(0, "id")
    TRUNAJOD_ORDERED_INDICES.append("nivel")
    TRUNAJOD_ORDERED_INDICES.append("grupo")
    for index, row in tqdm(dataset.iterrows(), desc="Textos Processados"):
        # print str(row["id"]) + ","
        # + process_text(row["texto"]) + ",{}".format(row["nivel"])
        try:
    ...
    writer = pd.ExcelWriter("/mnt/c/Users/dpalma/Desktop/trunajod_experimentos/features_trunajod.xlsx")

    ...
    def __init__(self, doc, lexico_semantic_norm_dict, lemmatizer=None):
        ...
        for token in doc:
            ...
            if word in lexico_semantic_norm_dict:
                ...
                concreteness += lexico_semantic_norm_dict[word].get("concreteness")
                imageability += lexico_semantic_norm_dict[word].get("imageability")
                context_availability += lexico_semantic_norm_dict[word].get("context_availability")
                familiarity += lexico_semantic_norm_dict[word].get("familiarity")
                count += 1
